...
**Widened 2026-08-27 — the underlying date cache now also feeds a display badge, not just this toast:** a small `📅N` badge (day-of-month) now renders in the company-name cell (between Name and Shares) whenever `alphapicks_earnings_dates` has a date 0-7 days out (`renderEarningsCalendarBadges()`, called at the end of `checkEarningsAlerts()`, idempotent). That needed real discovery beyond ±1 day, so both the FMP query's date range and the Nasdaq fallback's per-day query set were widened from a 3-day span (yesterday/today/tomorrow) to 9 days (yesterday through +7) — the ±1-day alert-firing logic itself is untouched, gated by its own separate check. Confirmed live: firing all 9 Nasdaq date-queries in one `Promise.all` overwhelmed the local proxy chain (first 3 succeeded, the other 6 ALL failed outright, even though the same +5-day query worked alone seconds later) — fixed by batching 3-at-a-time.

**Fixed same day — long company names hid the badge entirely:** `.company-name` truncates with `overflow:hidden`; the badge, appended as a plain sibling inside that same cell, shared the overflow container and got clipped whenever the name text alone nearly filled the cell (confirmed live: CRDO, "Credo Technology Group Holding Ltd"). Fixed by wrapping the actual name text in its own `.company-name-text` span carrying the truncation styles — only the name truncates now, the badge (`flex-shrink:0`) is always a fixed-size, fully-visible sibling regardless of name length.

**Fixed again same day — the fix above broke row vertical-centering:** the flex layout was first put directly on `.company-name` (the `<td>` itself), which overrides its `display` from the default `table-cell` to `flex` — this silently breaks the sheet-wide `td { vertical-align: middle }` rule (vertical-align only positions real table-cell boxes; a flex-display td just top-aligns instead), so every company name sat visibly higher than the rest of its row. Fixed by moving the flex layout onto a new `.company-name-inner` wrapper *inside* the `<td>` instead — the `<td>` stays a normal table-cell and keeps the row's default centering.
